...
export default function Waste() {
  ...
  const canEdit = hasPerm('waste.create');
        ...
        <table className="w-full text-sm text-left">
          <thead className="bg-muted text-muted-foreground border-b font-medium uppercase text-xs tracking-wider">
            <tr>
              ...
              {(hasPerm('waste.create') || hasPerm('waste.create')) && <th className="px-6 py-4 text-right">Actions</th>}
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            {isLoading ? (
              ...
              <tr key={w.id} className="table-row-hover">
                ...
                {(hasPerm('waste.create') || hasPerm('waste.create')) && (
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-1">
                      {hasPerm('waste.create') && <button onClick={() => openEdit(w)} className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground transition-colors" title="Edit"><Pencil size={14}/></button>}
                      {hasPerm('waste.create') && <button onClick={() => setDeleteConfirm({ id: w.id, name: w.ingredientName || w.menuItemName })} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-950/30 text-muted-foreground hover:text-red-600 transition-colors" title="Delete"><Trash2 size={14}/></button>}
                    </div>
                  </td>
                )}
              </tr>
