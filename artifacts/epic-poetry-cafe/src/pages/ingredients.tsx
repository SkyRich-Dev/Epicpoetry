  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Ingredients Master" description="Manage raw materials and their measurement units">
        {hasPerm('ingredients.edit') && <Button onClick={openCreate}><Plus size={18}/> Add Ingredient</Button>}
      </PageHeader>
        ...
        <table className="w-full text-sm text-left">
          <thead className="bg-muted text-muted-foreground border-b font-medium uppercase text-xs tracking-wider">
            <tr>
              ...
              {(canEdit || hasPerm('ingredients.edit')) && <th className="px-6 py-4 text-right">Actions</th>}
            </tr>
          </thead>
          <tbody className="divide-y divide-border" data-testid="ingredients-table-body">
            {isLoading ? (
              ...
              <tr key={item.id} className="table-row-hover">
                ...
                {(canEdit || hasPerm('ingredients.edit')) && (
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-1">
                      ...
                      {hasPerm('ingredients.edit') && <button onClick={() => setDeleteConfirm({ id: item.id, name: item.name })} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-950/30 text-muted-foreground hover:text-red-600 transition-colors" title="Delete"><Trash2 size={14}/></button>}
                    </div>
                  </td>
                )}
              </tr>
