...
function extractLink(kind: "create-password" | "reset-password" | "change-password"): string {
...
function extractTokenFromLink(url: string) {
...
before(async () => {
  ...
  smtpServer = new SMTPServer({
    ...
    onData(stream: SMTPServerDataStream, _session: SMTPServerSession, callback: (err?: Error | null) => void) {
      let raw = "";
  ...
  await new Promise<void>((resolve, reject) => {
    smtpServer!.listen(TEST_SMTP_PORT, "127.0.0.1", (err?: Error | null) => (err ? reject(err) : resolve()));
  });
...
after(async () => {
  ...
  if (smtpServer) {
    await new Promise<void>((resolve, reject) => smtpServer!.close((err?: Error | null) => (err ? reject(err) : resolve())));
  }
...
test("tenant owner is created without password and setup email is generated", async () => {
  const users = await queryJson<Array<{
    username: string;
    password_hash: string | null;
    password_set: boolean;
    email: string | null;
  }>>(
    `select username, password_hash, password_set, email from ${TEST_SCHEMA}.users where username = '${TEST_CUSTOMER_EMAIL}'`
  );
