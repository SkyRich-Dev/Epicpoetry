...
let baseUrl = "";
let integrationA: IntegrationRecord;
let integrationB: IntegrationRecord;
...
async function ensurePosIntegrationColumns(schemaName: string) {
  await runSql(`
...
async function provisionTenant(record: (typeof tenantRecords)[number]) {
  const payload = {
    source: "local-test",
    tenant: {
...
async function createIntegration(schema: string, restaurantId: string) {
  const payload = {
...
async function sendWebhook(integration: IntegrationRecord, payload: Record<string, unknown>) {
...
async function sendCompactWebhook(integration: IntegrationRecord, identifier: string, payload: Record<string, unknown>) {
...
async function invoiceRows(schemaName: string, invoiceNo: string) {
  return queryJson<Array<{
    id: number;
    invoice_no: string;
    source_type: string;
    ...
    `SELECT id, invoice_no, source_type, customer_name, customer_phone
     FROM ${schemaName}.sales_invoices
     WHERE invoice_no = '${invoiceNo}'`,
  );
}

async function invoiceLineCount(schemaName: string, invoiceNo: string) {
...
before(async () => {
  ...
  for (const record of tenantRecords) {
    ...
    const rows = await queryJson<Array<{ count: number }>>(`SELECT COUNT(*)::int AS count FROM ${record.schema}.sales_invoices`);
...
test("routes compact tenant hash and custom slug webhooks into the correct tenant schema", async () => {
  ...
  integrationA = { ...(patch.body as IntegrationRecord), webhookSecret: integrationA.webhookSecret };
