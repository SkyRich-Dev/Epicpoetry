...
let vendorId = 0;
let ingredientId = 0;
const createdS3Keys = new Set<string>();
...
function ownerToken(schema: string) {
...
async function queryJsonDirect<T>(sql: string): Promise<T> {
  const databaseUrl = process.env.DATABASE_URL;
  if (!databaseUrl) throw new Error("DATABASE_URL is not configured");
...
async function seedVendorAndIngredient() {
  ...
  const vendorRows = await queryJsonDirect<Array<{ id: number }>>(
    `with t as (
  ...
  const ingredientRows = await queryJsonDirect<Array<{ id: number }>>(
    `with t as (
...
async function createPurchaseRequest(body: RequestInit["body"], headers?: Record<string, string>) {
...
async function updatePurchaseRequest(id: number, body: RequestInit["body"], headers?: Record<string, string>) {
...
function purchasePayload(invoiceNumber: string) {
  return {
...
function trackAttachmentFromBody(body: any) {
  return body;
}

function extractStoredAttachment(raw: string | null | undefined): { key: string; name: string; type: string } | null {
...
test("upload image bill succeeds", async () => {
  ...
  const rows = await queryJson<Array<{ bill_attachment: string | null }>>(
    `select bill_attachment from ${TEST_SCHEMA}.purchases where id = ${Number(body.id)}`
  );
...
test("invalid file type fails", async () => {
  ...
  const rows = await queryJson<Array<{ count: number }>>(
    `select count(*)::int as count from ${TEST_SCHEMA}.purchases where invoice_number = 'ATT-BADTYPE-001'`
  );
...
test("file larger than 10 MB fails", async () => {
  ...
  form.append(
    "billAttachment",
    new File([new Uint8Array(MAX_TEST_ATTACHMENT_SIZE + 1)], "bill-big.pdf", { type: "application/pdf" }),
  );
  ...
  const rows = await queryJson<Array<{ count: number }>>(
    `select count(*)::int as count from ${TEST_SCHEMA}.purchases where invoice_number = 'ATT-TOOBIG-001'`
  );
  assert.equal(Number(rows[0]?.count || 0), 0);
});
...
test("edit purchase with new file and then remove attachment", async () => {
  ...
  const updatedRows = await queryJson<Array<{ bill_attachment: string | null }>>(
    `select bill_attachment from ${TEST_SCHEMA}.purchases where id = ${Number(created.id)}`
  );
  ...
  const removed: any = await removeResponse.json();
